...
function validRequest({ maxPage = 1, page = 1 }) {
  ...
  const validatedPage =
    Number.isInteger(page) && page > 0 ? Math.min(page, MAX_PAGE) : 1;
...
class MoviesServiceClass {
  async getGenres({ type = "movie", query }) {
    try {
      ...
      const data = {};
      await Promise.all(
        shuffledGenres.map(async (genre) => {
          const id = genre.id;
          // console.log("ID", id)
          const moviesByGenre = await this.getMovies({
            query: { with_genres: id, ...query },
            type,
          });
          // console.log(moviesByGenre);

          const shuffledMovies = moviesByGenre.result.data.results.sort(
            () => 0.5 - Math.random()
          );
          data[genre.name] = { id, genres: shuffledMovies };
        })
      );

      return ResFormatter.resForm({ data }, "You have got genres");
    } catch (error) {
  ...
  async getValidGenres({ type = "movie", query }) {
    try {
      const endpoint = `/genre/${type === "movie" ? "movie" : "tv"}/list`;
      const query = {
        language: "en",
      ...
      return ResFormatter.resForm({ ...genresData }, "You have got genres");
    } catch (error) {
      throwHandleError(error, "Server error while getting our genres");
    }
  }
